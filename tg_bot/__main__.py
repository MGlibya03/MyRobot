...
# ═══════════════════════════════════════════════════════════
# قائمة الكلمات اللي ما يردش عليها لو جت في جملة
# باش ما يتداخلش مع الكلام العادي
# ═══════════════════════════════════════════════════════════

SHORT_TRIGGERS = {
    "سلام", "باهي", "ههه", "لول", "توا", "كان", "شن", "شنو",
    "بوت", "غبي", "احمق", "هبل", "مهبول", "مجنون", "قمر",
    "حلو", "عسل", "غالي", "هاي", "هلا", "باي", "صاحي",
    "موجود", "ملل", "نعسان", "جوعان", "عطشان", "زعلان",
    "فرحان", "مريض", "تعبان", "زهقت", "شوية", "برشا",
    "زورو", "اسمك",
}
...
def start(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        update.effective_message.reply_text(
            "هلا! انا زورو 🤖\n"
            "اكتب مساعدة او اوامر باش تعرف اوامري!"
        )
...
def smart_reply(update: Update, context: CallbackContext):
    ...
    # ما يردش على الرسائل القصيرة جداً
    if len(text.strip()) < 2:
        return
    ...
    # ═══════════════════════════════════════════
    # مطابقة دقيقة اولا (النص كامل = الكلمة)
    # ═══════════════════════════════════════════
    if text_clean in SMART_REPLIES:
        try:
            message.reply_text(SMART_REPLIES[text_clean])
        except:
            pass
        return

    # ═══════════════════════════════════════════
    # مطابقة الجمل الطويلة (اكثر من كلمتين)
    # بس الكلمات اللي مش قصيرة
    # ═══════════════════════════════════════════
    for trigger, response in SMART_REPLIES.items():
        # الكلمات القصيرة لازم تكون مطابقة دقيقة
        if trigger in SHORT_TRIGGERS:
            continue

        # الجمل الطويلة (مثل "السلام عليكم") ممكن تكون جزء من النص
        if len(trigger) > 5 and trigger in text_clean:
            try:
...
def send_settings(chat_id, user_id, user=False):
    if user:
        ...
        if CHAT_SETTINGS:
            ...
            dispatcher.bot.send_message(
                user_id,
                text="⚙️ اي وحدة تبي تفحص اعداداتها لـ '{}'?".format(chat_name),
                reply_markup=InlineKeyboardMarkup(
...
def settings_button(update: Update, context: CallbackContext):
    ...
    try:
        if mod_match:
            ...
            query.message.edit_text(
                text="⚙️ اي وحدة تبي تفحص اعداداتها؟",
                reply_markup=InlineKeyboardMarkup(
        ...
        if excp.message not in ["Message is not modified", "Query_id_invalid", "Message can't be deleted"]:
            log.exception("خطأ في settings_button: %s", str(query.data))
...
def error_handler(update: Update, context: CallbackContext):
    try:
        ...
        log.warning("خطأ: غير مصرح")
    except BadRequest as e:
        log.warning("طلب خاطئ: %s", str(e))
    except TimedOut:
        log.warning("خطأ: انتهت المهلة")
    except NetworkError:
        log.warning("خطأ في الشبكة")
    except ChatMigrated as e:
        log.warning("القروب انتقل لـ %s", e.new_chat_id)
    except TelegramError as e:
        log.warning("خطأ تيليجرام: %s", str(e))
...
def migrate_chats(update: Update, context: CallbackContext):
    ...
    log.info("انتقال من %s لـ %s", str(old_chat), str(new_chat))
...
def main():
    # تسجيل الاوامر الاساسية - عربي بدون / + انجليزي بـ /
    start_handler = CustomCommandHandler(
        ["start", "ابدا", "بداية"], start, run_async=True
    )
    help_handler = CustomCommandHandler(
        ["help", "مساعدة", "مساعده", "اوامر", "الاوامر"], help_command, run_async=True
    )
    stats_handler = CustomCommandHandler(
        ["stats", "احصائيات", "الاحصائيات"], stats, run_async=True
    )
    ...
    # الردود الذكية (اقل اولوية - group 99)
    ...
    log.info("🤖 زورو بوت شغّال توا! 🇱🇾")
    log.info("👨‍💻 المبرمج: @%s", OWNER_USERNAME)
    log.info("✅ الاوامر العربية بدون / مفعّلة!")

    if WEBHOOK:
        log.info("يستخدم webhooks...")
...
if __name__ == "__main__":
    ...
    log.info("تم تحميل الموديولات بنجاح: " + str(ALL_MODULES))
    main()

...
anon_reply_text = "🔒 يبدو انك مجهول الهوية!\nاضغط الزر تحت باش تثبت هويتك:"
...
def user_is_not_admin_errmsg(msg: Message, permission: AdminPerms = None, cb: CallbackQuery = None):
    if permission and permission.value:
        errmsg = f"⛔ ما عندك الصلاحية المطلوبة:\n`{permission.value}`!"
    else:
        errmsg = f"⛔ ما عندك الصلاحية اللازمة لهذا الامر!"
    if cb:
...
def button_expired_error(u: Update):
    errmsg = f"❌ هذا الزر منتهي الصلاحية!"
    if u.callback_query:
